...
export class RequestsEditComponent implements OnInit {
  ...
  submitRequest() {
    ...
    this.serviceItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.commentedServiceItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.orgItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.commentedOrgItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.protocolItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.commentedProtocolItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.accessItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
    this.commentedAccessItems.forEach(i => { RequestsEditComponent.pushAttr(i, perunAttributes); });
  ...
  private filterAndSortArrays() {
    this.commentedServiceAttrs = RequestsEditComponent.filterAndSort(this.commentedServiceAttrs);
    this.commentedOrganizationAttrs = RequestsEditComponent.filterAndSort(this.commentedOrganizationAttrs);
    this.commentedProtocolAttrs = RequestsEditComponent.filterAndSort(this.commentedProtocolAttrs);
    this.commentedAccessControlAttrs = RequestsEditComponent.filterAndSort(this.commentedAccessControlAttrs);

    this.serviceAttrs = RequestsEditComponent.filterAndSort(this.serviceAttrs);
    this.organizationAttrs = RequestsEditComponent.filterAndSort(this.organizationAttrs);
    this.protocolAttrs = RequestsEditComponent.filterAndSort(this.protocolAttrs);
    this.accessControlAttrs = RequestsEditComponent.filterAndSort(this.accessControlAttrs);
  }

  private pushInputs() {
    this.clearArrays();
    this.request.serviceAttrs().forEach((attr, _) => {
      RequestsEditComponent.pushInput(attr, this.commentedServiceAttrs, this.serviceAttrs);
    });

    this.request.organizationAttrs().forEach((attr, _) => {
      RequestsEditComponent.pushInput(attr, this.commentedOrganizationAttrs, this.organizationAttrs);
    });

    this.request.protocolAttrs().forEach((attr, _) => {
      RequestsEditComponent.pushInput(attr, this.commentedProtocolAttrs, this.protocolAttrs);
    });

    this.request.accessControlAttrs().forEach((attr, _) => {
      RequestsEditComponent.pushInput(attr, this.commentedAccessControlAttrs, this.accessControlAttrs);
    });
  }
